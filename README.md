...
Imported recordings are validated before replay, including event types, page references, secret variable declarations, and values used in generated code. The server rejects overlapping recording transitions and permits only one enrichment at a time. `record_status` exposes the recorder phase: `idle`, `starting`, `recording`, or `stopping`.

MCP request cancellation and server shutdown cancel enrichment; the standalone enrichment commands also handle SIGINT/SIGTERM. Cancellation preserves the original script until publication and releases the session lock. On POSIX, cancellation and timeouts terminate the child process group, including browser descendants; Windows currently terminates only the direct child. A report becomes `verified` only after the script is published. If final report writing fails after publication, the returned result includes a warning; inspect the published script and retained backup before retrying.
