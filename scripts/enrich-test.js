...
try {
  ...
  if (values.help) {
    ...
    const result = await enrichReplay({scriptPath:positionals[0],recordingPath:positionals[1],workspace:values.workspace,maxAttempts:Number(values['max-attempts'])},{signal:termination.signal});
