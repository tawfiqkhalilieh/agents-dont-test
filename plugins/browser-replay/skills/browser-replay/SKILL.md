...
5. If the user requests automatic assertions, call `record_enrich` with the exported script and recording paths after stopping. This invokes the Antigravity Flash assertion-enricher and replays the task up to seven times, so use a resettable test target within the user's authorized scope. It requires authenticated `agy`. Report failures accurately; only a `verified` result means the candidate passed.
6. On later runs of the same task, inspect the saved JavaScript and run it with Node instead of repeating browser exploration. Replay only within the user's requested scope. A replay repeats side effects.
