...
import { runProcess, checkCancelled } from './process.js';
import { parseRecording } from './recording-schema.js';
...
export async function invokeAntigravity({workspace,prompt,timeoutMs,signal}) {
  let result;
  try {
    result = await runProcess(process.env.BROWSER_REPLAY_AGY || 'agy', ['--agent','assertion-enricher','--output-format','json','--print-timeout',`${Math.ceil(timeoutMs/1000)}s`,'-p',prompt], {cwd:workspace,timeoutMs,signal});
  } catch(error) {
...
export async function enrichReplay({scriptPath,recordingPath,workspace = process.cwd(),maxAttempts = 3}, {invokeAgent = invokeAntigravity, executionTimeoutMs = 60000, agentTimeoutMs = 180000, signal} = {}) {
  ...
  const recording = parseRecording(JSON.parse(await readFile(recordingPath,'utf8')));
  ...
  try {
    ...
    const observed = await runProcess(process.execPath,[observationScript],{cwd:workspace,env,timeoutMs:executionTimeoutMs,maxOutput:1000000,signal});
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const agent = await invokeAgent({workspace,prompt,candidatePath,recordingPath,observationsPath,attempt,timeoutMs:agentTimeoutMs,signal});
      checkCancelled(signal);
      ...
      try {
        ...
        verification = await runProcess(process.execPath,[candidatePath],{cwd:workspace,env,timeoutMs:executionTimeoutMs,signal});
        ...
        await writeFile(reportPath,JSON.stringify({status:'ready-to-publish',scriptPath,recordingPath,observationsPath,backupPath,attempts:history},null,2),{mode:0o600});
        ...
        let warning;
        await writeFile(reportPath,JSON.stringify({status:'verified',scriptPath,recordingPath,observationsPath,backupPath,attempts:history},null,2),{mode:0o600}).catch(error => {warning = `Verified script published, but report could not be finalized: ${redact(error.message)}`;});
        return {status:'verified',scriptPath,recordingPath,observationsPath,backupPath,reportPath,attempts:attempt,...checks,...(warning ? {warning} : {})};
      } catch(error) {
        checkCancelled(signal);
    ...
    error.message = redact(error.message)+(workDir ? `\nArtifacts: ${workDir}` : '');
    throw error;
  } finally {
    await lock.release();
  }
